...
#[test]
fn genuinely_empty_request_system_replays_as_empty_message() {
    // `Request.system: Some(vec![])` is the caller's own data: it replays
    // as an empty-content `system` message, silently (current-behavior
